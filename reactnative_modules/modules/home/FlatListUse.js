...
export default class FlatListUse extends Component {
  ...
  componentDidMount() {
    ...
    for (i=0;i<20;i++) {
      if (i%2 === 0){
        text = '何塞·保罗·贝塞拉·马希尔·儒尼奥尔（José Paulo Bezerra Maciel Júnior），简称保利尼奥[1]';
      } else {
        text = '2009年，凭借《倾城之恋》入围韩国首尔电视节，唯一一位亚洲女演员获 韩国首尔电视节最佳女演员提名。' +
  ...
  renderItem = ({item,index}) => {
    ...
    return (
        <View style={{justifyContent:'center'}}>
          <Text style={{marginTop:15,marginLeft:15,marginBottom:15}} onLayout={this.titleLayout.bind(this)}>
            {'第'+item.title}
          </Text>
        </View>
    )
